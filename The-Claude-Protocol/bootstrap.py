...
def install_beads(project_dir: Path, project_name: str, claude_only: bool = False) -> bool:
    ...
    # Emit Dolt connection descriptor for the Kanban UI.
    # Must run after `bd init` so `.beads/config.yaml` has the database name.
    print("  - Writing Kanban UI connection descriptor...")
    write_kanban_config(project_dir, project_name)
...
def _manual_beads_init(beads_dir: Path):
    ...
    Fallback path when `bd init` is unavailable. Dolt is the only supported
    storage backend in modern beads, so this only seeds metadata — it does NOT
    create an issues.jsonl placeholder (doing so misleads the Kanban UI into
    treating the empty file as the data source instead of Dolt).
    """
    ...
    print("  - Created .beads manually (Dolt will be initialized on first `bd init`)")


def _resolve_dolt_port() -> Optional[int]:
    """Read the global Dolt server port from ~/.beads/dolt-server.port.

    Returns None if the file is missing or unreadable.
    """
    port_file = Path.home() / ".beads" / "dolt-server.port"
...
def _resolve_beads_database(beads_dir: Path, project_name: str) -> str:
    """Resolve the Dolt database name beads uses for this project.

    Prefers the `database:` field in `.beads/config.yaml` (written by `bd init`).
    Falls back to a slugified project name.
    """
...
def write_kanban_config(project_dir: Path, project_name: str) -> bool:
    """Emit `.beads/kanban.json` describing how the Kanban UI should reach Dolt.

    The Kanban UI reads this file to connect directly to the Dolt MySQL-wire
    server instead of tailing the legacy `issues.jsonl`. Returns True on
    success, False if the Dolt server isn't reachable (port file missing).
    """
    beads_dir = project_dir / ".beads"
    port = _resolve_dolt_port()
    if port is None:
        print("  - WARNING: ~/.beads/dolt-server.port not found; skipping kanban.json")
        print("    The Kanban UI will not have a Dolt connection descriptor.")
        print("    Run `bd doctor` to diagnose, then re-run bootstrap.")
        return False

    database = _resolve_beads_database(beads_dir, project_name)
    config = {
        "backend": "dolt",
        "host": "127.0.0.1",
        "port": port,
    ...
    kanban_path = beads_dir / "kanban.json"
    kanban_path.write_text(json.dumps(config, indent=2) + "\n")
    print(f"  - Wrote {kanban_path.relative_to(project_dir)} (Dolt @ 127.0.0.1:{port}, db={database})")
    return True
...
def main():
    ...
    if not claude_only:
        ...
        if not install_beads(project_dir, project_name, claude_only=False):
        ...
        if not install_beads(project_dir, project_name, claude_only=True):
            print("\nERROR: Beads CLI is required. Aborting bootstrap.")
            sys.exit(1)
