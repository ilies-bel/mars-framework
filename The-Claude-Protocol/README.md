...
Since modern `beads` stores data in Dolt (not JSONL), bootstrap emits `.beads/kanban.json` — a connection descriptor the Kanban UI reads to talk to the local Dolt MySQL-wire server directly. See [docs/kanban-config.md](docs/kanban-config.md) for the schema and troubleshooting.

---

## Getting Started

**Zero-interaction install (recommended):**

```bash
npx -y beads-orchestration@latest setup
```

Run it at the root of the project you want to bootstrap. It copies the skill to `~/.claude/skills/`, runs the bootstrap (claude-only mode, auto-detected Kanban UI), and exits. No prompts, no questions.

Override defaults with flags:

```bash
npx -y beads-orchestration@latest setup --project-dir /path/to/project --external-providers --with-kanban-ui
```

Then restart Claude Code and run `/create-beads-orchestration` once — it will detect the bootstrap is done and dispatch the discovery agent to create supervisors for your tech stack.
...
The skill is non-interactive: it auto-infers project name/dir and skips external-provider setup unless you explicitly ask for it.
