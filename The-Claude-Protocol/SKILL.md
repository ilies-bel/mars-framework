...
- Bootstrap writes `.beads/kanban.json` so the UI connects to Dolt (the beads storage backend) directly instead of a stale JSONL file — see [docs/kanban-config.md](docs/kanban-config.md)

**Without `--with-kanban-ui`:**
- Worktrees created via raw git commands
- `.beads/kanban.json` is still emitted; the UI will use it if you start one later
